...
# water can only move to the same level or a lower level. (By level, we mean 
# the total height of the terrain plus any water in that column.)
# Since moving left will eventually make it fall, it moves left. (A droplet 
# "made to fall" means go to a lower height than it was at previously.) 
...
# preferred to move left, even though it could move right (and moving right makes 
# it fall quicker.)
...
for i in range(volume):
    ...
    if index != k: # we might need to check right still
        add 1 to heights[index]
        continue

    while index + 1 <= len(heights) - 1 and heights[index + 1] < heights[index]:
        index += 1
    heights[index] += 1
    index = k
